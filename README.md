# 👋 Hi, I'm Dishan Gimhan!
### 💻 Full-stack Developer | 🌱 AI/ML & Open Source Enthusiast | 🧠 Lifelong Learner

I'm a passionate developer currently pursuing a BSc in IT at SLIIT 🇱🇰. I work as an Associate Web Developer at Aradhana Global Group, where I build scalable web apps with the MERN stack. I love combining AI with real-world impact—recently working on an elephant movement prediction system to reduce human-elephant conflict in Sri Lanka 🐘🌍.

---

- 🔭 I’m currently working on:  
  `AI-based Elephant Corridor Prediction System` (ML + GIS + MERN)  
  `Movie Explorer App` with MERN stack + TMDb API  
  `Inventory Management Dashboard` (React + Spring Boot)

- 🌱 I’m currently learning:  
  `Deep Learning (TensorFlow/PyTorch)` | `Next.js` | `Advanced MongoDB Aggregation`

- 👯 I’m looking to collaborate on:  
  AI-powered systems, open-source MERN tools, geospatial tech, or any socially impactful projects

- 💬 Ask me about:  
  React.js, Node.js, Express, MongoDB, REST APIs, Tailwind, Git & GitHub

- 📫 How to reach me:  
  [![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?logo=linkedin&logoColor=white)](https://linkedin.com/in/dishangimhan)  
  📧 Email: kdgimhan@gmail.com *(replace with your real email)*

- ⚡ Fun fact:  
  I’m building tech solutions for elephants… and one day, I want to launch a startup that empowers students & early devs with real-world projects 🚀

---

### 📊 GitHub Stats
![Dishan's GitHub stats](https://github-readme-stats.vercel.app/api?username=dishangimha24n&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=dishangimhan24&layout=compact&theme=radical)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=dishangimhan24&theme=radical)

---

### 🏆 GitHub Trophies
[![trophy](https://github-profile-trophy.vercel.app/?username=dishangimhan24&theme=algolia)](https://github.com/ryo-ma/github-profile-trophy)

---

### 📂 Featured Projects

- 🚀 [Elephant Corridor AI](https://github.com/your-repo) – Predicts elephant movement using ML + map visualization
- 🎬 [Movie Explorer App](https://github.com/your-repo) – MERN app with TMDb integration and user auth
- 📦 [Inventory System](https://github.com/your-repo) – Spring Boot backend + React frontend for real-time data tracking

---
![Dishan's GitHub stats](https://github-readme-stats.vercel.app/api?username=dishangimhan24&show_icons=true&theme=radical)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=dishangimhan24&layout=compact&theme=radical)
![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=dishangimhan24&theme=radical)

> "Learning by building" is my motto. Let’s connect and build something awesome together!
